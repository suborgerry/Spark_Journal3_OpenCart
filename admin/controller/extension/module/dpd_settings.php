...
class ControllerExtensionModuleDpdSettings extends Controller {
	...
	public function index() {
        $data['MODULE_DPD_SETTINGS_VERSION'] = "1.1";
		...
		$data['manifest_list'] = $this->model_shipping_dpdlivehandler->getManifest();
		...
		if (!$this->config->get('dpd_setting_module_version')){
            $this->model_setting_setting->editSetting('dpd_setting',['dpd_setting_module_version' => $data['MODULE_DPD_SETTINGS_VERSION']]);
        }
	...
	public function print_manifest() {
		...
		if ($pdf_content) {
			$this->load->model('shipping/dpdlivehandler');
			return $this->model_shipping_dpdlivehandler->printManifest($pdf_content);
		}
	}
	...
	public function getTracking() {
		...
		if (isset($order_id) && $order_id) {
			$requestResult = $this->model_shipping_dpdlivehandler->getParcelStatus($order_id);
			$order_barcode = $this->model_shipping_dpdlivehandler->getOrderBarcode($order_id);
				...
				foreach ($requestResult as $result) {
					if (isset($result['status']) && strtolower($result['status']) == 'ok') {
						if ($result['parcel_status'] != '') {
							...
							$this->model_shipping_dpdlivehandler->setOrderHistory($order_id, $success_message);
						} else {
	...
	public function submit_orders(){
		...
		if(isset($action) != '') {
			if ($action == 'print_dpd_manifests') {
				$result = $this->model_shipping_dpdlivehandler->close_manifest();
				...
				$result = $this->model_shipping_dpdlivehandler->shipment_creation($this->request->post['selected']);
				...
				if ($tracking_numbers) {
					$this->model_shipping_dpdlivehandler->print_parcel_label($tracking_numbers);
				}
				...
				foreach ($this->request->post['selected'] as $order_id) {
					$result = $this->model_shipping_dpdlivehandler->getOrderBarcode($order_id);
				...
				if ($tracking_numbers) {
					$result = $this->model_shipping_dpdlivehandler->cancel_Shipment($tracking_numbers);

					if ($result['status'] == 'err') {
						$error_message .= $this->language->get('text_error_response') .'<strong>' . $result['errlog'] . '</strong>';

						foreach ($this->request->post['selected'] as $order_id) {
							$this->model_shipping_dpdlivehandler->setOrderHistory($order_id, $this->language->get('text_error_response') . ' ' . $result['errlog']);
						...
						foreach ($this->request->post['selected'] as $order_id) {
							$this->model_shipping_dpdlivehandler->setOrderHistory($order_id, $this->language->get('text_dpd_order_successfully_canceled'));
	...
	public function requestCourier (){
		...
		if ($params) {
			...
			// Callcourier
			$result = $this->model_shipping_dpdlivehandler->requestCourier($params);
	...
	public function collectionReqesut() {
		...
		if (isset($postParams) && $postParams) {
			...
			if (empty($json['error'])) {
				$requestCollection = $this->model_shipping_dpdlivehandler->requestCollection($postParams);
	...
	public function reverseCollectionRequest() {
		...
		if (isset($this->request->get['modal_ajax']) && $this->request->get['modal_ajax'] == 1) {
			...
			$response = $this->model_shipping_dpdlivehandler->reverseCollectionRequest($order_id, $warehouse_id);
	...
	public function updatePrintingLabelsNo() {
		...
		if ($order_id && $labels_number > 0) {
			$this->model_shipping_dpdlivehandler->setLabelsNumber($order_id, $labels_number);
	...
	public function updateRodServices() {
		...
		$this->model_shipping_dpdlivehandler->updateRodServices($order_id, $return_documents);
	}
	...
	public function updateRodNote() {
		...
		if ($order_id) {
			$this->model_shipping_dpdlivehandler->updateRodNote($order_id, $note);
