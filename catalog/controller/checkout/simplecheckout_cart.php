    ...
    public function index() {
        ...
        foreach ($products as $product) {
            ...
            foreach ($product['option'] as $option) {
                //CarMod
                if(in_array($option['name'],Array('Code','Date','Source','Supplier'))){continue;}
                /////////////////////////
                if ($version >= 200) {
                    if ($option['type'] != 'file') {
                        $value = $option['value'];
...
//            if(isset($product['CarMod']) AND $product['CarMod']=="Y"){
//                $image=$product['image'];
//            }
            ...
            if ($version >= 200) {
                ...
                $this->_templateData['products'][] = array(
                    ...
                    'thumb'     => (isset($product['CarMod']) AND $product['CarMod']=="Y")?$product['image'].'" style="width:'.$product['image_full_width'].';':$image,
//                    'thumb'     => $image,
                    'name'      => $product['name'],
                    'model'     => $product['model'],
                   ...
                   // 'href'      => $this->url->link('product/product', 'product_id=' . $product['product_id']),
                    'href'      => (isset($product['CarMod']) AND $product['CarMod']=="Y")?$product['product_url']:$this->url->link('product/product', 'product_id=' . $product['product_id']),
                    'CarMod'    =>(isset($product['CarMod']) AND $product['CarMod']=="Y")?'Y':'',
                );
            } elseif ($version >= 156) {
                $profile_description = '';
        ...
        $this->_templateData['currency'] = $this->session->data['currency'];
        ...
        $this->_templateData['total_last'] = [array_pop($totals)];
        $this->_templateData['total_up'] = $totals;
