...
class ModelExtensionShippingDpdParcel extends Model {
	...
	public function getQuote($address) {
		...
		foreach ($query->rows as $result) {
			...
			// Disable this shipping method if weight is more than 20kg
			...
			if ($status) {
				...
				if ($cost == 0) {
					...
					$text = $this->currency->format($this->tax->calculate($cost, $this->config->get('shipping_dpd_parcel_cod_status'), $this->config->get('config_tax')), $this->session->data['currency']);
				}
