...
class ModelExtensionTotalCodfee extends Model {
    public function getTotal($total) {
        ...
        if ((($payment_method && ($payment_method == 'cod')) && ($sub_total > $this->config->get('cod_total')) ) && ($sub_total > 0)) {
            ...
            if ($this->config->get('config_cod_fee_tax_class_id')) {
                $tax_rates = $this->tax->getRates($this->config->get('cod_fee_fee'), $this->config->get('cod_fee_tax_class_id'));
            ...
            $total['total'] += $this->config->get('cod_fee_fee');
